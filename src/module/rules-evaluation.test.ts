...
test('All conditions joined by "and" must hold for a rule to fire.', () => {
  const engine = rules({
    '{$.approved}': {
      default: 'no',
      rules: {
        'if {$.age} greater/equals 18 and {$.hasId} equals true': {
          result: 'yes',
        },
      },
    },
  ...
  assert.strictEqual(
    engine.run({ age: 20, hasId: true }).result.approved,
    'yes',
  )
  assert.strictEqual(
    engine.run({ age: 20, hasId: false }).result.approved,
    'no',
  )
...
test('A condition can compare two state references.', () => {
  ...
  assert.strictEqual(
    engine.run({ value: 5, min: 1, max: 10 }).result.inRange,
    true,
  )
  assert.strictEqual(
    engine.run({ value: 10, min: 1, max: 10 }).result.inRange,
    true,
  )
  assert.strictEqual(
    engine.run({ value: 50, min: 1, max: 10 }).result.inRange,
    false,
  )
})
...
test('The against value keeps its spaces, so multi word comparisons work.', () => {
  ...
  assert.strictEqual(
    engine.run({ name: 'Obi Wan Kenobi' }).result.rank,
    'Master',
  )
...
test('greater is exclusive and greater/equals is inclusive at the boundary.', () => {
  const engine = rules({
    '{$.gt}': {
      default: 'no',
      rules: { 'if {$.n} greater 10': { result: 'yes' } },
    },
    '{$.gte}': {
      default: 'no',
...
test('excludes is the negation of includes and holds when the value cannot contain anything.', () => {
  ...
  assert.strictEqual(
    engine.run({ roles: ['user', 'editor'] }).result.safe,
    true,
  )
  assert.strictEqual(
    engine.run({ roles: ['user', 'banned'] }).result.safe,
    false,
  )
