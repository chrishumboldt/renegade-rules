...
test('A dependency declared through the "against" side of a condition is still resolved first.', () => {
  ...
  assert.strictEqual(engine.parse['{$.withinLimit}'].dependencies?.[0], '{$.limit}');
  assert.strictEqual(engine.run({ vip: true, spent: 300 }).result.withinLimit, true);
  assert.strictEqual(engine.run({ vip: false, spent: 300 }).result.withinLimit, false);
});
...
test('A shared dependency in a diamond graph runs exactly once.', () => {
  const engine = rules({
    ...
    '{$.a}': { default: 0, rules: { 'if {$.seed} equals true': { result: 20 } } },
  });
  ...
  assert.strictEqual(
    output.rulesRun.filter(key => key === '{$.a}').length,
    1,
  );
});
...
test('When a cycle is found, the run aborts and returns the partial result.', () => {
  const engine = rules({
    '{$.a}': { default: 'a-default', rules: { 'if {$.b} equals x': { result: 'a2' } } },
    '{$.b}': { default: 'b-default', rules: { 'if {$.a} equals y': { result: 'b2' } } },
  });
